...
public class StreamEx3 {
  ...
  public static void main(String[] args) {
    ...
    // 수학점수 모으기
    // stream.map(s -> s.getMath()).forEach(System.out::println);
    ...
    // 스트림 문자열을 대문자로 변경 후 출력
    // stream2.map(String::toUpperCase).forEach(System.out::println);

    // 스트림 문자열을 대문자로 변경 후 리스트로 돌려받기

    List<String> strList = stream2
    ...
    // 최종 연산의 결과를 Optional 객체에 담아서 반환 => 널을 체크하기 위한 if문 사용 안함
